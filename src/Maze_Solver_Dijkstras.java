...
class Solve_Maze_Dijkstra implements MazeSolverMetrics {
    ...
    Solve_Maze_Dijkstra(int[][] maze) {
        if (Solve_Maze(maze)) {
            Maze_Utils.Replace_Ones(maze);
            Maze_Utils.Print_Maze(maze);
        } else {
            System.out.println("No solution found using Dijkstra's");
        }
    }
    ...
    public static boolean Solve_Maze(int[][] maze) {
        ...
        List<Cell> path = Find_Path_Dijkstra(maze, rows, cols);
    ...
    public static List<Cell> Find_Path_Dijkstra(int[][] maze, int rows, int cols) {
        Cell start = new Cell(0, 0);
        Cell end = new Cell(rows - 1, cols - 1);

        PriorityQueue<Cell> openSet = new PriorityQueue<>(Comparator.comparingInt(cell -> cell.cost));
        openSet.add(start);
        ...
        while (!openSet.isEmpty()) {
            ...
            for (Cell neighbor : Maze_Utils.Get_Neighbors(current, rows, cols, maze)) {
                ...
                if (!costSoFar.containsKey(neighbor) || newCost < costSoFar.get(neighbor)) {
                    ...
                    neighbor.cost = newCost + Heuristic(neighbor, end);
